...
export class CurriculumGradeSubjectsComponent {
  ...
  constructor(private notifier: NotifierService, 
    ...
    {
      this.titleService.setTitle('CurriculumCompletion' + ' | Glentree Academy :: Curriculum Portal');
    ...
    async getSubjects(index : number)
    {
      if(index < this.grades.length)
      {
        try
        {
          let response = await this.userService.getTeachSubjects(this.loginUser.uuid, this.grades[index].id).toPromise();
